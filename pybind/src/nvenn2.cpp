...
are in columns (1) or rows (2)")
    ...
    .def("simulate", &borderLine::pysim, py::arg("maxlevel") = 1, "Generates Venn diagram figure. If maxlevel is set to a value higher than 0, it does so \
using the exhaustive mode. Larger values for maxlevel tend to give better results, but the computational resources needed grow extremely fast. \
See estimate_exhaustive_run_time() to use this mode safely.")
