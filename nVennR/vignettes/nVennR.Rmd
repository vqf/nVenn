...
Representing proportional Venn diagrams with more than two sets is not trivial, as the number of potential regions grows exponentially with the number of sets. The `nVenn` algorithm represents areas as circles with the desired areas and then encloses those circles in curves to create the sets. The resulting diagrams are approximately proportional and can represent an arbitrary number of sets.

# Input

To make a Venn diagram, nVennR provides the `nVennDiagram` function. Its input is either a list of lists, a text with the shape of a table or a previously generated nVenn object. 
...
The most straightforward way to change a set color is by using `setVennColor`. WARNING: colors must be formatted as valid svg color expressions. If we pass an invalid svg color, the resulting figure may be broken or another color can be set.
...
There are also functions to change several colors at once. First, `nVenn` has four pre-packaged color palettes (`0`-`3`). The key to understand the behavior of `nVennR` is that it first applies a palette and then individual colors. This means that set colors take precedence over palettes. Therefore, if we now apply a different palette, `Set2` will still be black.
...
We can also rotate the plot with `rotateVenn`. The angle is interpreted in degrees and it is applied counter-clockwise.
